...
int (timer_subscribe_int)(uint8_t *bit_no) {
  ...
  *bit_no = BIT(hook_); // máscara a utilizar
  if (sys_irqsetpolicy(TIMER0_IRQ, IRQ_REENABLE, &hook_) != 0) return 1; // subscreve interrupções
  return 0;
}

int (timer_unsubscribe_int)() {
  if (sys_irqrmpolicy(&hook_) != 0) return 1; // desubscreve interrupções
  return 0;
}
 
void (timer_int_handler)() { // timer 0 interrupt handler
  counter_++; // increments counter
}
