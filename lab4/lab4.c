...
message msg;
  ...
  unsigned current_time = idle_time;
  
  if (mouse_enable_data_reporting2()) return 1;

  if (subscribe_mouse(&hook_mouse)) return 1;
  if (timer_subscribe_int(&hook_timer)) return 1;
  
    while (current_time > 0u) {
    if ((r = driver_receive(ANY, &msg, &ipc_status)) != 0) {
      printf("driver_receive failed with: %d", r);
      continue;
    }
    ...
    if (is_ipc_notify(ipc_status)) { 
      switch (_ENDPOINT_P(msg.m_source)) {
        case HARDWARE:
          ...
          if (msg.m_notify.interrupts & hook_timer) {
            timer_int_handler();
            if (count_timer % sys_hz() == 0) {
              current_time--;
            }
          }   
        default:
